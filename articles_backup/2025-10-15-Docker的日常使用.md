...
updated_at: "2025-10-17 05:02:32"
issue_number: 25
labels: ['docker']
...
|docker cp <容器ID>:/容器内路径 /宿主机路径|从容器复制到宿主机|
|docker cp /宿主机路径 <容器ID>:/容器内路径|从宿主机复制到容器|
|docker cp 容器ID:/app/checkpoints/. ./checkpoints/|#复制整个目录的内容（使用 /. 表示目录内所有内容）|
...
| `docker compose logs` | 查看所有服务的聚合日志。-f 实时查看 |
