---
title: "Python 命令行之旅：深入 click 之命令篇"
date: 2022-01-07T14:43:08+08:00
description: ""
image: ""
categories: [Coding]
tags: [python,click]
...
每当命令被调用时，`click` 会创建新的上下文，并链接到父上下文。通常，我们是看不到上下文信息的。但我们可以通过 [pass\_context](https://click.palletsprojects.com/en/7.x/api/#click.pass_context) 装饰器来显式让 `click` 传递上下文，此变量会作为第一个参数进行传递。
...
*   将 `cli` 定义为了链式命令组，并且指定 invoke\_without\_command=True，也就意味着可以不传子命令来触发命令组
